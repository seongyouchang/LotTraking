...
namespace Form_list
{
    public partial class WF_performance : Form
    {
        ...
        public WF_performance(string sWID)
        {
            InitializeComponent();
            WID3 = sWID;
        }
        ...
        private void WF_performance_Load(object sender, System.EventArgs e)
        {
            ...
            dtGrid.Columns.Add("WorkerID",   typeof(string));       //작업자 아이디
            dtGrid.Columns.Add("WorkerName", typeof(string));     //작업자명
            dtGrid.Columns.Add("FEA",        typeof(string));     //생산수량
        ...
        private void btnfair_Click(object sender, EventArgs e)  //양품등록 클릭시
        {
            ...
            try
            {
                ...
                Adapter = new SqlDataAdapter("WF_performance_S", Connect2);
        ...
        private void button6_Click(object sender, EventArgs e) //추가버튼
        {
            ...
            if (WID3 == "")
            {
                MessageBox.Show("작업지시 선택을 하지않았습니다.");
                return;
            }

            if (MessageBox.Show("생산실적을 등록하시겠습니까?",
            "생산시작",
            ...
            try
            {
                ...
                // 선택한 Row의 데이터를 가져온다
                string sFea =     dgvr.Cells[3].Value.ToString();
                string sFairea =  dgvr.Cells[4].Value.ToString();
                string sErrorea = dgvr.Cells[5].Value.ToString();


                cmd = new SqlCommand();
                cmd.Connection = Connect2;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "WF_performance_U";


                cmd.Parameters.AddWithValue("WID", WID3);
                cmd.Parameters.AddWithValue("FEA", sFea);
                cmd.Parameters.AddWithValue("FAIREA", sFairea);
                cmd.Parameters.AddWithValue("ERROREA", sErrorea);
